...
class NytimeArticleFetcher(ArticleFetcher):
    ...
    def _extract_plain_content(self, html):
    ...
    def _html_to_infomation(self, html, link, date):
        ...
        try:
            ...
            try:
                ...
                content = self._extract_plain_content(html)
        except Exception as err:
            return None

        return {
            'title': title,
            'published_date': published_date if published_date else date.strftime('%Y-%m-%d %H:%M:%S'),
            #'authors': authors,
